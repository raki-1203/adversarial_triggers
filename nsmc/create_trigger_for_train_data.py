...
def main(args):
    ...
    run = wandb.init(
        ...
        name=f'train_data-{"positive to negative" if args.dataset_label_filter else "negative to positive"}'
             f'-trigger-{args.num_trigger_tokens}',
        config=wandb_config,
        reinit=True,
    )
    ...
    targeted_train_df = train_df[train_df['label'] == dataset_label_filter]
    print('postive to negative' if args.dataset_label_filter == 1 else 'negative to positive',
          f'targeted_train_df 개수 : {len(targeted_train_df)}')
    ...
    # valid label setting
    targeted_train_label = targeted_train_df['label'].tolist()

    # tokenizing dataset
    tokenized_targeted_train = tokenized_dataset(targeted_train_df, tokenizer, args)

    # make dataset
    targeted_valid_dataset = SentimentClassificationDataset(tokenized_targeted_train, targeted_train_label)
    ...
    for epoch in range(args.epoch):
        ...
        for batch in tqdm(DataLoader(targeted_valid_dataset, batch_size=args.universal_perturb_batch_size,
            ...
            run.log({f'train_data-{"positive to negative" if args.dataset_label_filter else "negative to positive"}'
    ...
    filename = 'train_data-trigger_accuracy_{}word_{}_dict'.format(args.num_trigger_tokens,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--universal_perturb_batch_size', type=int, default=70,
                        help='batch size per device during evaluation (default: 70)')
